...
export default function WastePage() {
  ...
  const stats = [
    { label: 'Total Registros', value: waste.length, color: '#ddeee5' },
    { label: 'Reciclables',     value: recyclable,    color: '#34d399' },
    { label: 'No Reciclables',  value: nonRecyclable, color: '#fb923c' },
  ];
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '14px' }}>
        {stats.map(({ label, value, color }) => (
          <div
            ...
            style={{
              borderRadius: '12px',
              background: 'linear-gradient(145deg, rgba(16,185,129,0.04) 0%, transparent 100%)',
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '14px' }}>
              ...
              {waste.length === 0 ? (
                <tr>
                  <td colSpan={4} style={{ padding: '56px 20px', textAlign: 'center' }}>
                    <RiRecycleLine size={36} style={{ color: '#1e3a2a', margin: '0 auto 10px', display: 'block' }} />
                    <p style={{ margin: 0, color: '#3a5445', fontSize: '13.5px', fontWeight: '600', fontFamily: ff }}>Sin registros de residuos</p>
                    <p style={{ margin: '4px 0 0', color: '#1e3a2a', fontSize: '12px', fontFamily: ff }}>Las clasificaciones aparecerán cuando el endpoint esté disponible</p>
                  </td>
                </tr>
              ) : (
                waste.map((w, i, arr) => (
                  <tr key={w.waste_id} style={{ borderBottom: i < arr.length - 1 ? '1px solid rgba(16,185,129,0.05)' : 'none' }}>
                    <td style={{ padding: '12px 20px', color: '#c8ddd4', fontSize: '13px', fontFamily: ff }}>{w.recognized_item}</td>
                    <td style={{ padding: '12px 20px' }}>
                      <Badge color={w.type === 'RECICLABLE' ? 'emerald' : 'orange'} size="xs">{w.type}</Badge>
                    </td>
                    <td style={{ padding: '12px 20px', color: '#3a5445', fontSize: '13px', fontFamily: ff }}>{w.username ?? '—'}</td>
                    <td style={{ padding: '12px 20px', color: '#2d4038', fontSize: '11.5px', fontFamily: ffMono }}>{w.timestamp}</td>
                  </tr>
                ))
              )}
            </tbody>
        ...
        <div style={{ ...panelStyle, padding: '20px' }}>
          <p style={{ margin: 0, color: '#c8ddd4', fontWeight: '600', fontSize: '14px', fontFamily: ff }}>Distribución</p>
          <p style={{ margin: '3px 0 0', color: '#2d4038', fontSize: '11.5px', fontFamily: ff }}>Por tipo de residuo</p>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '32px 10px', textAlign: 'center' }}>
            ...
            >
              <RiLeafLine size={26} style={{ color: '#2d4038' }} />
            </div>
            <p style={{ margin: 0, color: '#2d4038', fontSize: '13px', fontWeight: '600', fontFamily: ff }}>Gráfico disponible</p>
            <p style={{ margin: '5px 0 0', color: '#1e3a2a', fontSize: '11.5px', fontFamily: ff, lineHeight: 1.5 }}>
